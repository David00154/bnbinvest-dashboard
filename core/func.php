...
$UserDetailsAssoc = null;
if (isLogged()) {
    $user = $_SESSION['logged'];
    $stmt = $royaldb->query("SELECT * FROM user WHERE username='$user'") or die($royaldb->error);
    if ($stmt->num_rows > 0) {
        $UserDetailsAssoc = $stmt->fetch_assoc();
    }
}
...
function totalAdminBalanceh() {
    global $royaldb;

    $stmt = $royaldb->query("SELECT btc, bnb, blur, pi, ada, eth, usdt, xrp, op  FROM user") or die($royaldb->error);
    if($stmt->num_rows > 0) {
        $bal = 0;
...
function totalBalance(string $person) {
    global $royaldb;

    $stmt = $royaldb->query("SELECT btc, bnb, blur, pi, ada, eth, usdt, xrp, op  FROM user WHERE id='$person'") or die($royaldb->error);

    if($stmt->num_rows > 0) {
        $bal = 0;
        foreach ($stmt->fetch_object() as $key => $value) {
            if($value != NULL) {
                $bal += (int)$value;
            }
        }
