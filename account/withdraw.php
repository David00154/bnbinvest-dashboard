...
if ($depo->num_rows > 0) {
  ...
  while ($load = $depo->fetch_object()) {
    ...
    // $getdepo .= "<tr><td>$sn</td><td>$" . number_format($load->amount) . "</td>$load->asset_from<td>" ."</td><td>$load->method</td><td>$status</td><td>$getWdate</td></tr>";
    $getdepo .= "
      <tr>
        <td>$sn</td>
        <td>$". number_format((float)$load->amount) ."</td>
        <td>$load->asset_from</td>
        <td>$load->method</td>
        <td>$status</td>
        <td>$getWdate</td>
      </tr>
...
if (isset($_POST['withdraw'])) {
  ...
  $asset_from = ucfirst($_POST['asset_from']);
  ...
  // }  
  $bal = (int)$UserDetailsAssoc[lcfirst($asset_from)] - $amount;
  if ($withdrawal_method == "Bitcoin") {
    $btc_address = $_POST['btc_address'];
    $w = $royaldb->query("UPDATE user SET $asset_from='$bal' WHERE id=$UserDetails->id") or die($royaldb->error);
    $wu = $royaldb->query("INSERT INTO withdrawal SET amount=$amount, asset_from='$asset_from', user_id=$UserDetails->id, receiver='$btc_address', date=$time") or die($royaldb->error);
  } else if ($withdrawal_method == "PayPal") {
    $paypal_email = $_POST['paypal_email'];
    $w = $royaldb->query("UPDATE user SET $asset_from='$bal' WHERE id=$UserDetails->id") or die($royaldb->error);
    $wu = $royaldb->query("INSERT INTO withdrawal SET amount=$amount,  method='PayPal', asset_from='$asset_from', user_id=$UserDetails->id, receiver='$paypal_email', date=$time") or die($royaldb->error);
  } else {
    ...
    $w = $royaldb->query("UPDATE user SET $asset_from='$bal' WHERE id=$UserDetails->id") or die($royaldb->error);
    $wu = $royaldb->query("INSERT INTO withdrawal SET amount=$amount, asset_from='$asset_from', user_id=$UserDetails->id, method='$nmethod', receiver='$response', date=$time") or die($royaldb->error);
  }
...
?>
<title>Withdrawal | BnbInvest Fx </title>
          ...
          <div class="box-body">
            <form class="form" method="post">
              ...
              <div class="form-group">
                ...
                <input class="form-control border-primary" type="number" name="amount" placeholder="0.00" id="amount" min="500" max="10000" required>
              </div>

               <div class="form-group">
                <label for="withdrawal_method">Select The Asset You Wish To Withdraw From</label>
                <select name="asset_from" class="form-control" id="withdrawalMethod" required>
                  <option value="">--Select Method--</option>
      ...
      <div class="col-lg-6">
        <div class="box box-solid">
          ...
          <div class="box-body">
            ...
            At BnbInvest Fx, we have designed our withdrawal process to be as easy as our funding process. To begin the withdrawal process first select your preferred withdrawal method and then type in the amount you want to withdraw and click "Proceed". <br><br>

            <h6 style="color: #1E9FF2;"><strong>What Methods Are There For Withdrawal Of Funds? </strong></h6>
            BnbInvest Fx provides three(3) withdrawal methods (Bitcoin, PayPal and Direct Bank Transfer). <br><br>

            <h6 style="color: #1E9FF2;"><strong>Must Withdrawal Requests Only Be Made At Certain Times? </strong></h6>
            Requests for withdrawals can be made at any time via the BnbInvest Fx website. The requests will be processed immediately, and during the relevant financial institution's business hours.<br><br>
